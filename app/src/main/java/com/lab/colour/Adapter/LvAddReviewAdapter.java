...
/**
 * Created by SeoHyeonBae on 2016-09-28.
 */
public class LvAddReviewAdapter extends BaseAdapter{
    private Context mContext = null;
    private Vector<Restaurant> restaurants = new Vector<>();
    CustomHolder holder;
    int nowPosition;

    public LvAddReviewAdapter(Context mContext) {
        super();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if ( convertView == null ) {
            ...
            convertView = inflater.inflate(R.layout.listitem_add_review, null);

            holder = new CustomHolder();
            holder.holder_image = (ImageView) convertView.findViewById(R.id.iv_add_review_image);
            holder.holder_rating_bar =(AppCompatRatingBar) convertView.findViewById(R.id.rb_add_review);
            holder.holder_rating = (TextView) convertView.findViewById(R.id.tv_add_review_rating);
            holder.holder_restaurant_name = (TextView) convertView.findViewById(R.id.tv_add_review_restaurant_name);
            holder.holder_address = (TextView) convertView.findViewById(R.id.tv_add_review_address);
            holder.holder_review_cnt = (TextView) convertView.findViewById(R.id.tv_add_review_review_cnt);
            holder.holder_distance = (TextView) convertView.findViewById(R.id.tv_add_review_distance);

            //like
            holder.holder_like = (ImageButton) convertView.findViewById(R.id.ib_add_review_like);
            ...
            holder.holder_rating_bar.setIsIndicator(false);           //true - 별점만 표시 사용자가 변경 불가 , false - 사용자가 변경가능

            holder.holder_rating_bar.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
                @Override
                public void onRatingChanged(RatingBar ratingBar, float rating, boolean fromUser) {
                    if(fromUser){
                        Toast.makeText(mContext, String.valueOf(rating), Toast.LENGTH_SHORT).show();
                        restaurants.get(position).setRating(rating);
                        notifyDataSetChanged();
                    }
